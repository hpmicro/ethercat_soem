...
int bfin_emac_send(void *packet, int length)
{
    ...
    desc.tx_desc_list_cur->tdes2_bm.buffer1 = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t)packet);
...
#else
    sta = enet_check_received_frame(&desc.rx_desc_list_cur, &desc.rx_frame_info);
    if (1 == sta)
    {
